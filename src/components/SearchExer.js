...
const SearchExer = ({ setExercise, bodyPart, setBodyPart }) => {
  ...
  const handleSearch = async () => {
    if (inField) {
      ...
      const searchedExer = exercisesData.filter(
        (exercise) =>
          exercise.name.toLowerCase().includes(inField) ||
    ...
    <Stack alignItems="center" mt="37px" justifyContent="center" p="20px">
      ...
      <Box position="relative" mb="72px">
        <TextField
          ...
          height="76px"
          value={inField}
          onChange={(e) => setInField(e.target.value.toLowerCase())}
